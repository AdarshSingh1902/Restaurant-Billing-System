...
def additem_screen():
    ...
    add_item=Button(frm,command=lambda:addItem_db(),width=8,text="Add",font=('',20,'bold'),bd=10)
    reset_btn=Button(frm,command=lambda:reset_home(),width=8,text="reset",font=('',20,'bold'),bd=10)
    ...
    add_item.place(relx=.35,rely=.45)
    reset_btn.place(relx=.5,rely=.45)
    ...
    def logout():
        option=messagebox.askyesno('confirmation','you want to logout?')
        if(option==True):
            frm.destroy()
            home_screen()
...
def billing_screen():
    ...
    welcome_lbl=Label(frm,text="Welcome,Admin For Bill",font=('',20),bg='pink')
    welcome_lbl.place(x=1,y=14)
    ...
    item_entry.place(relx=.13,rely=.2)
    
    qty_label.place(relx=.05,rely=.3)
    qty_entry.place(relx=.13,rely=.3)
